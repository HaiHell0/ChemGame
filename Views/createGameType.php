  ...
  if (count($_POST) > 0) {
    ...
    if (!is_dir($newFileName) && !$_POST["game-name"] == "") {
      ...
      try {
        ...
        $gameTypeItem->addGame($_SESSION["ADMIN_ID"], $newFileName, $dirPhp, $dirCss, $dirJs);
        echo '<script>alert("New game type created");window.location.href = "index.php";</script>';
      } catch (Exception $e) {
        echo '<script>alert("An error has occurred");window.location.href = "index.php";</script>';
      }
